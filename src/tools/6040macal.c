...
void calibrate(const uint8_t node_id, const uint8_t ch)
{
  ...
  snprintf(reg, sizeof(reg), "ch%u.mode", ch);

  reg_access.type = WLMIO_REGISTER_VALUE_UINT8;
  reg_access.length = 1;
  memcpy(reg_access.value, &(uint8_t[]){1U}, 1);
  ...
  printf("Apply 4 mA to the input of CH%u and press any key", ch);
  getc(stdin);
  ...
  printf("Apply 20 mA to the input of CH%u and press any key", ch);
  getc(stdin);
  ...
  snprintf(reg, sizeof(reg), "ch%u.ma_cal", ch);
  reg_access.length = 2;
