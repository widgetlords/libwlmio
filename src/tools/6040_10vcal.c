...
void calibrate(const uint8_t node_id, const uint8_t ch)
{
  ...
  reg_access.length = 1;
  memcpy(reg_access.value, &(uint8_t[]){2U}, 1);
  ...
  printf("Apply 2 V to the input of CH%u and press any key", ch);
  getc(stdin);
  ...
  printf("Apply 10 V to the input of CH%u and press any key", ch);
  getc(stdin);
  ...
  snprintf(reg, sizeof(reg), "ch%u.10v_cal", ch);
  reg_access.length = 2;
