...
void calibrate_current_source(const uint8_t node_id, const uint8_t ch)
{
	assert(ch > 0 && ch <= 4);
	...
	uint8_t param = 1;
	int ret = wlmio_execute_command_sync(node_id, 0, &param, 1);
	printf("ret: %d\n", ret);
	printf("Measure the output of CH%u as a current source in uA and enter it: ", ch);
	ret = scanf("%u", &low);
	if(ret <= 0)
	{
	...
	char reg[51];
	snprintf(reg, sizeof(reg), "ch%u.calibration.source", ch);
...
void calibrate_current_sink(const uint8_t node_id, const uint8_t ch)
{
	assert(ch > 0 && ch <= 4);
	...
	int ret = wlmio_execute_command_sync(node_id, 0, &param, 1);
	printf("ret: %d\n", ret);
	printf("Measure the output of CH%u as a current sink in uA and enter it: ", ch);
	ret = scanf("%u", &low);
	if(ret <= 0)
	{
		printf("Failed to parse value\n");
	...
	char reg[51];
	snprintf(reg, sizeof(reg), "ch%u.calibration.sink", ch);
...
int main(int argc, char** argv)
{
	...
	{
		calibrate_current_source(node_id, i);
		calibrate_current_sink(node_id, i);
	}
