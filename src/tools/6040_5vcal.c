...
void calibrate(const uint8_t node_id, const uint8_t ch)
{
  ...
  reg_access.length = 1;
  memcpy(reg_access.value, &(uint8_t[]){0U}, 1);
  ...
  printf("Apply 1 V to the input of CH%u and press any key", ch);
  getc(stdin);
  ...
  printf("Apply 5 V to the input of CH%u and press any key", ch);
  getc(stdin);
  ...
  snprintf(reg, sizeof(reg), "ch%u.5v_cal", ch);
  reg_access.length = 2;
